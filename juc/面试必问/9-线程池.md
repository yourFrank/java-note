...
![image-20200317152541284](https://image.imxyu.cn/file/image-20200317152541284.png)
...
![image-20200317175202647](https://image.imxyu.cn/file/image-20200317175202647.png)

![image-20200317175241007](https://image.imxyu.cn/file/image-20200317175241007.png)
...
- Executors.newScheduledThreadPool(int corePoolSize)：线程池支持定时以及周期性执行任务，创建一个corePoolSize为传入参数，最大线程数为整型的最大数的线程池
...
![image-20200317182004293](https://image.imxyu.cn/file/image-20200317182004293.png)

我们可以看到线程池的内部，还使用到了LinkedBlockingQueue 链表阻塞队列

同时在查看Executors.newCacheThreadPool 看到底层用的是 SynchronousBlockingQueue阻塞队列

最后查看一下，完整的三个创建线程的方法	![image-20200317183202992](https://image.imxyu.cn/file/image-20200317183202992.png)



## 线程池的重要参数

![image-20200317183600957](https://image.imxyu.cn/file/image-20200317183600957.png)
...
![image-20200317201150197](https://image.imxyu.cn/file/image-20200317201150197.png)
...
![image-20200318154414717](https://image.imxyu.cn/file/image-20200318154414717.png)
...
- 线程池不允许使用Executors去创建，而是通过 ThreadPoolExecutor 的方式，这样的处理方式让写的同学更加明确线程池的运行规则，规避资源耗尽的风险
  - Executors返回的线程池对象弊端如下：
    - FixedThreadPool和SingleThreadPool：
      - 运行的请求队列长度为：Integer.MAX_VALUE，因为线程数只有这些，可能会堆积大量的请求，队列爆满从而导致OOM
    - CacheThreadPool和ScheduledThreadPool
      - 运行的请求队列长度为：Integer.MAX_VALUE，因为每次队列一满就会创建新的非核心线程，可能导致线程数上限太大导致oom
