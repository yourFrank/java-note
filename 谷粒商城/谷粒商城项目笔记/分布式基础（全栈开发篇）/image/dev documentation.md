...
> 这个名字是根据镜像仓库中镜像的名字，叫什么就写什么就行
>
> ![image-20220415200226832](image-20220415200226832.png)

默认会在这init 的路径下初始化一个vagrantfile文件

![image-20220415200714377](image-20220415200714377.png)

运行vagrant up即可启动虚拟机。系统root用户的密码是vagrant

这里下载会很慢，直接复制网址到网页上进行下载

![image-20220415201507412](image-20220415201507412.png)

执行命令 **vagrant box add centos/7 CentOS-7-x86_64-Vagrant-2004_01.VirtualBox.box** 添加本地box

add后面的centos/7和我上面初始化的名字一样，一定要一样哦

然后**vagrant up** 启动

打开virtual-box，能看到新加了一个centos7的虚拟机

![image-20220415201751250](image-20220415201751250.png)

此时已经运行好了该虚拟机，ctrl + c 停掉窗口就行

![image-20220415202004174](image-20220415202004174.png)



vagrant其他常用命令

vagrant ssh 自动使用 vagrant 用户连接虚拟机，使用exit 可以退出

![image-20220415202326893](image-20220415202326893.png)



1、右键虚拟机退出，可以正常关机

2、第二天之后可以在virtualbox中启动该虚拟机，也可以使用vagrant up , 然后vagrant ssh 连接（需要注意的是使用vagrant 命令需要在目录下有vagrantfile文件的目录下运行，初始化默认在用户目录，并且推荐使用git命令窗口运行，不要用cmd 可能会有问题） 

![image-20220415202622940](https://image.imxyu.cn/file/image-20220415202622940.png)

![image-20220415202825125](https://image.imxyu.cn/file/image-20220415202825125.png)
...
默认虚拟机中的连接方式是网络地址转换的方式，需要对每个端口进行端口映射，这样很麻烦

![image-20220415203616260](https://image.imxyu.cn/file/image-20220415203616260.png)

![image-20220415203638374](https://image.imxyu.cn/file/image-20220415203638374.png)

直接修改 Vagrantfile ，修改成和windows同段的ip ,然后vagrant reload

  config.vm.network "private_network", ip: "192.168.56.10"

这里 ip 需要在 物理机下使用 ipconfig 命令找到，可以看到virtual box有一个虚拟ip，我们直接修改文件，指定和这个ip同段即可

![image-20201015141629387](image-20201015141629387.png)

![image-20220415204119372](https://image.imxyu.cn/file/image-20220415204119372.png)

两边能互相ping 通即可，我的windows ip是1.106 ，那个56.1是分配个virtual box的ip，56.10是vitrual box中centos的ip

![image-20220415204255874](https://image.imxyu.cn/file/image-20220415204255874.png)
...
新建仓库，选择分支模型为master/develop ，主分支和开发分支，平时在开发分支上开发

![image-20220415212530279](https://image.imxyu.cn/file/image-20220415212530279.png)
